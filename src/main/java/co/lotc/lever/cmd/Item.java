...
public class Item extends BaseCommand {
	...
	public void invoke(Player p, ItemStack is, @Arg("amount") @Default("1") @Range(min=1, max=64) int amount) {
		validate(getSender().hasPermission("lever.item.admin") || (is.getItemMeta() == null || is.getItemMeta().getAttributeModifiers().size() < 1), "Cannot create an item that has attributes");
		is.setAmount(amount);
